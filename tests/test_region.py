...
class TestCrossingLine:
    def test_crossing_line_with_horizontal_segment_is_always_false(self) -> None:
        assert Region.line_crossing(0, 0, ((0, 0), (1, 0))) == Region.Crossing.NONE

    def test_x_less_than_min_segment_x_is_not_crossing(self) -> None:
        assert Region.line_crossing(0, 0, ((1, 0), (1, 1))) == Region.Crossing.NONE

    def test_x_greater_than_max_segment_x_is_crossing(self) -> None:
        assert Region.line_crossing(2, 0, ((1, 0), (1, 1))) == Region.Crossing.CROSS

    def test_x_with_segment_x_percentage_less_than_y_percentage_is_not_crossing(self) -> None:
        assert Region.line_crossing(0.5, 0.75, ((0, 0), (1, 1))) == Region.Crossing.NONE
        assert Region.line_crossing(0, 0, ((1, 0), (0, 1))) == Region.Crossing.NONE

    def test_x_with_segment_x_percentage_more_than_y_percentage_is_crossing(self) -> None:
        assert Region.line_crossing(0.75, 0.5, ((0, 0), (1, 1))) == Region.Crossing.CROSS

    def test_x_with_segment_x_percentage_equal_to_y_percentage_is_crossing(self) -> None:
        assert Region.line_crossing(0.5, 0.5, ((0, 0), (1, 1))) == Region.Crossing.ON

    def test_crossing_line_at_its_end_is_crossing(self) -> None:
        assert Region.line_crossing(1, 0, ((1, 0), (0, 1))) == Region.Crossing.ON
...
class TestGridPoints:
    ...
    def test_less_than_whole_x_range_yields_only_points_within_range(
        ...
        boundary_points = [(0, 0), (4, 0), (4, 4)]
        region = Region(boundary_points, top, bottom, cell_size=1)
        points = region.get_1d_grid_points(y=2)
        assert points == [2, 3, 4]

    def test_hitting_only_single_boundary_point_yields_the_point(
        self, top: HeightFunc, bottom: HeightFunc
    ) -> None:
        boundary_points: list[XYPoint] = [(0, 0), (1, 0), (0, 1)]
        region = Region(boundary_points, top, bottom, cell_size=1)
    ...
    def test_gap_in_boundary_does_still_ensure_full_1d_grid_coverage(
        self, top: HeightFunc, bottom: HeightFunc
    ) -> None:
        # M-shaped boundary
        boundary_points: list[XYPoint] = [(0, 5), (0, 0), (4, 4), (8, 0), (8, 5)]
        region = Region(boundary_points, top=top, bottom=bottom, cell_size=2)
        points = region.get_1d_grid_points(y=1)
